...
const useStartRide= ()=>{
    ...
    const {data,error,isLoading,mutate:start,mutateAsync:startAsync}:useStartRideType= useMutation({
        mutationKey:["rides","ride","ride","start",ride?.id??""], 
        mutationFn:async()=>{
            const response = await axios.put(`${SERVER_URL}/rides/ride/start/${ride?.id??""}`,{},config)
